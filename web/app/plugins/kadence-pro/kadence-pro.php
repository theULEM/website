...
update_option( 'kadence_cloud_activation',                              'Activated' );
update_option( 'kadence_conversions_activation',                        'Activated' );
update_option( 'kt_api_manager_kadence_fullpane_slider_activated',      'Activated' );
update_option( 'kt_api_manager_kadence_pricing_table_activated',        'Activated' );
update_option( 'kt_api_manager_kadence_woo_template_builder_activated', 'Activated' );
	...
	add_filter( 'pre_http_request', function( $pre, $args, $url ) {
		if ( strpos( $url, 'https://api.startertemplatecloud.com/wp-json/kadence-starter/v1/get/' ) !== false ) {
			...
			return wp_remote_get(
				"http://wordpressnull.org/kadence/starter-templates/{$query['request']}.json",
				[
					'sslverify' => false,
					'timeout' => 25
				]
