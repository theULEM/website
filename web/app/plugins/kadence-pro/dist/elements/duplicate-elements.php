<?php
/**
 * Class Kadence_Pro\Duplicate_Elements
 *
 * @package Kadence Pro
 */
...
class Duplicate_Elements {
	...
	public function dupe_link( $actions, $post ) {
		...
		if ( ! current_user_can( apply_filters( 'kadence_duplicate_post_capability', 'manage_options' ) ) ) {
			return $actions;
		}
		...
		$actions['duplicate'] = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=' . $this->slug . '&action=kadence_duplicate_' . $this->slug . '&amp;post=' . $post->ID ), 'kadence_duplicate_post_' . $post->ID ) . '" aria-label="' . esc_attr__( 'Make a duplicate from this.', 'kadence-pro' )
			. '" rel="permalink">' . esc_html__( 'Duplicate', 'kadence-pro' ) . '</a>';
	...
	public function duplicate_action() {
		if ( empty( $_REQUEST['post'] ) ) {
			wp_die( esc_html__( 'No post to duplicate has been supplied!', 'kadence-pro' ) );
		}
		...
		if ( false === $clone_post ) {
			...
			wp_die( sprintf( esc_html__( 'Post creation failed, could not find original post: %s', 'kadence-pro' ), esc_html( $post_id ) ) );
		}
