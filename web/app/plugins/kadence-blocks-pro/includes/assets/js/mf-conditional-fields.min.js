...
const mfConditionalFields = (forms, options = {}) => {
	...
	let self = {
		...
		evaluateRule: (rule, formIndex) => {
			...
			if (triggers[formIndex].includes(name)) {
				...
				if (trigger.length > 0) {
					...
					} else if ( ( triggerType == 'date' || triggerType == 'time' ) && trigger.value) {
						// Special logic for handling time and date fields.
						// Their target value can be a full date time instead of just a time or date so may need to be stripped
						if ( triggerType == 'date' ) {
							// Parse the datetimes as to strip out the time component.
							let triggerValueEpoch = Date.parse(trigger.value + 'Z');
							let valueEpoch = Date.parse(value + 'Z');
							triggerValue = triggerValueEpoch - (triggerValueEpoch % 86400000);
							value = valueEpoch - (valueEpoch % 86400000);
						} else if ( triggerType == 'time' ) {
							// Parse the datetimes as to strip out the date component.
							let triggerValueEpoch = Date.parse(trigger.value);
							if ( ! triggerValueEpoch ) {
								// HTML time inputs can sometimes be not parseable without an attached.
								triggerValueEpoch = Date.parse('1970-01-01' + ' ' + trigger.value + 'Z')
							}
							let valueEpoch = Date.parse(value + 'Z');
							triggerValue = triggerValueEpoch % 86400000;
							value = valueEpoch % 86400000;
						}
					} else {
		...
		compareValues: (operator, searchVal, targetVal) => {
			console.log('compareValues',operator, searchVal, targetVal)
...
if (typeof (window) !== 'undefined') {
	// Gather all the conditional fields
	const conditionalFields = document.querySelectorAll("[data-conditional-rules]");
	let conditionalForms = [];

	// Loop over them to find conditional forms
	for (let i = 0; i < conditionalFields.length; i++) {
		const conditionalField = conditionalFields[i];
		
		const conditionalForm = conditionalField.closest('form');

		if ( conditionalForm ) {
			conditionalForms.push( conditionalForm );
		}
	}
	const uniqueConditionalForms = [...new Set(conditionalForms)];

	// Activate the library on those forms
	mfConditionalFields( uniqueConditionalForms, {
		disableHidden: true
	} );
