...
class Kadence_Blocks_Pro_Abstract_Block {
	...
	public function render_css( $attributes, $content, $block_instance ) {
		...
		if ( isset( $attributes['uniqueID'] ) ) {
			...
			if ( ! $css_class->has_styles( 'kb-' . $this->block_name . $unique_style_id ) && ! is_feed() && apply_filters( 'kadence_blocks_pro_render_inline_css', true, $this->block_name, $unique_id ) ) {
				$css        = $this->build_css( $attributes, $css_class, $unique_id, $unique_style_id );
				if ( ! empty( $css ) && ! wp_is_block_theme() ) {
					if ( 'slide' === $this->block_name ) {
						$this->render_styles_footer( 'kb-' . $this->block_name . $unique_style_id, $css );
					} else {
						$content = '<style>' . $css . '</style>' . $content;
					}
				}
			} elseif ( ! wp_is_block_theme() && ! $css_class->has_header_styles( 'kb-' . $this->block_name . $unique_style_id ) && ! is_feed() && apply_filters( 'kadence_blocks_pro_render_inline_css', true, $this->block_name, $unique_id ) ) {
