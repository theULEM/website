...
class Kadence_Blocks_Pro_Custom_Icons {
	...
	public function metabox_admin_scripts( $hook ) {
		...
		if ( 'kb_icon' === $typenow ) {
			...
			wp_register_script(
				...
				KBP_URL . 'includes/assets/js/kb-custom-icons-admin.min.js',
				array( 'jquery' ),
				KBP_VERSION,
				true
			);
	...
	public function render_metabox() {
		...
		wp_register_script( 'kb-pro-icon-upload', KBP_URL . 'dist/icon-upload.js', array( 'lodash', 'wp-components', 'wp-dom-ready', 'wp-element', 'wp-i18n' ), '2.2', true );
		wp_localize_script(
			'kb-pro-icon-upload',
			...
			array(
				...
				'ajax_loader'    => KBP_URL . 'includes/assets/images/ajax-loader.gif',
	...
	public function add_editor_block_icons() {
		...
		if ( $icons ) {
			...
			wp_enqueue_script( 'kadence_blocks_custom_icons', KBP_URL . 'includes/assets/js/kb-custom-icons.min.js', array( 'wp-blocks', 'wp-i18n', 'wp-element' ), KBP_VERSION, true );
			wp_localize_script(
